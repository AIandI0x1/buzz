...
const overrides = new Map([
  ...
  ["src-tauri/src/commands/messages.rs", 720], // + NIP-17 encrypted send routing (encrypted_recipients picks NEWEST 39002 across relays + fails loudly if no other members resolve, no silent self-only encrypt) // + encrypted threaded replies (in-rumor NIP-10 thread tags, no plaintext leak) // feed multi-query + NIP-50 search + forum thread resolution + thread ref + reactions via REQ + edit_message media_tags param (Slack-style attachment-editable edits)
